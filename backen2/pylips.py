# version 1.3.2 - FIXED for Android TV pairing
import platform    
import subprocess
import configparser
import requests
...
class Pylips:
    def __init__(self, ini_file):
        ...
        # Force HTTPS/1926 if host is provided and no credentials are set
        if args.host and (len(self.config["TV"]["user"]) == 0 or len(self.config["TV"]["pass"]) == 0):
            self.config["TV"]["protocol"] = "https://"
            self.config["TV"]["port"] = "1926"
            print("DEBUG: Forced protocol to HTTPS and port to 1926 for pairing attempt.")
        ...
        if len(sys.argv)>1:
            ...
            if args.user and args.password:
                ...
                self.config["TV"]["port"] = "1926"
                self.config["TV"]["protocol"] = "https://"
            # REMOVED THE PROBLEMATIC EARLY CHECK - LET find_api_version HANDLE IT
            if len(args.apiv) != 0:
                self.config["TV"]["apiv"]=args.apiv

        # check API version and handle pairing
        if len(self.config["TV"]["apiv"])==0:
            if self.find_api_version(self.verbose):
                # After finding API version, check if credentials are missing
                if len(self.config["TV"]["user"]) == 0 or len(self.config["TV"]["pass"]) == 0:
                    print("Credentials missing. Forcing HTTPS/1926 for pairing...")
                    self.config["TV"]["protocol"] = "https://"
                    self.config["TV"]["port"] = "1926"
                    self.pair()
        ...
        if (len(sys.argv)==1 or (len(sys.argv)==3 and sys.argv[1] == "--config")) and self.config["DEFAULT"]["mqtt_listen"] == "True":
            ...
            print("Setup completed! You can now use Pylips with commands or enable MQTT mode.")
    ...
    def find_api_version(self, verbose=True, possible_api_versions=[6,5,1]):
        if verbose:
            print ("Checking API version and port...")
        
        # Define possible protocols and their default ports
        possible_combinations = [
            ("https://", 1926),
            ("http://", 1925)
        ]

        for protocol, port in possible_combinations:
            for api_version in possible_api_versions:
                try:
                    url = f"{protocol}{self.config['TV']['host']}:{port}/{api_version}/system"
                    if verbose:
                        print(f"Trying {url}")
                    r = session.get(url, verify=False, timeout=2)
                ...
                if r.status_code == 200:
                    try:
                        ...
                        if verbose:
                            print(f"Received non-JSON response for {url}. Status: {r.status_code}")
                        continue # Skip if not JSON, as we expect system info

                    if "api_version" in response_json:
                        self.config["TV"]["apiv"] = str(response_json["api_version"]["Major"])
                    else:
                        if verbose:
                            print(f"Could not find a valid API version in response for {url}! Pylips will try to use '{api_version}'" )
                        self.config["TV"]["apiv"] = str(api_version)
                    
                    # Update protocol and port based on successful connection
                    self.config["TV"]["protocol"] = protocol
                    self.config["TV"]["port"] = str(port)

                    if "featuring" in response_json and \
                       "systemfeatures" in response_json["featuring"] and \
                       "pairing_type" in response_json["featuring"]["systemfeatures"] and \
                       response_json["featuring"]["systemfeatures"]["pairing_type"] == "digest_auth_pairing":
                        # If pairing type is digest_auth_pairing, ensure HTTPS and port 1926 are set
                        # This might override if we found it on HTTP first, which is correct for pairing.
                        self.config["TV"]["protocol"] = "https://"
                        self.config["TV"]["port"] = "1926"
                        if verbose:
                            print(f"Detected digest_auth_pairing. Setting protocol to https:// and port to 1926.")
                    
                    if verbose:
                        print(f"API version found: {self.config['TV']['apiv']}, Protocol: {self.config['TV']['protocol']}, Port: {self.config['TV']['port']}")
                    return True
        return False
