...
# Nuevo modelo para la respuesta de discover
...
class PhilipsTVController:
    ...
    async def send_key(self, key: str) -> Dict[str, Any]:
        ...
        except httpx.RequestError as e: # Captura errores de httpx
            logger.error(f"Error de conexión al enviar tecla {key} a {self.tv_ip}: {e}")
            return {
                "success": False,
                "message": f"Error de conexión al enviar tecla {key}: {str(e)}"
            }
        except Exception as e:
            logger.error(f"Error inesperado al enviar tecla {key}: {e}")
            return {
                "success": False,
                "message": f"Error inesperado al enviar tecla {key}: {str(e)}"
            }
    ...
    async def get_power_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error de conexión al obtener estado de {self.tv_ip}: {e}")
            return {
                "success": False,
                "message": f"Error de conexión al obtener estado: {str(e)}"
            }
        except Exception as e:
            logger.error(f"Error inesperado al obtener estado: {e}")
            return {
                "success": False,
                "message": f"Error inesperado al obtener estado: {str(e)}"
            }
    ...
    async def get_volume_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error de conexión al obtener volumen de {self.tv_ip}: {e}")
            return {
                "success": False,
                "message": f"Error de conexión al obtener volumen: {str(e)}"
            }
        except Exception as e:
            logger.error(f"Error inesperado al obtener volumen: {e}")
            return {
                "success": False,
                "message": f"Error inesperado al obtener volumen: {str(e)}"
            }
    ...
    async def cast_url_to_browser(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error de conexión al enviar URL {url} a {self.tv_ip}: {e}")
            return {
                "success": False,
                "message": f"Error de conexión al enviar URL: {str(e)}"
            }
        except Exception as e:
            logger.error(f"Error inesperado al enviar URL {url}: {e}")
            return {
                "success": False,
                "message": f"Error inesperado al enviar URL: {str(e)}"
            }
...
@router.post("/discover", response_model=List[SmartTV])
async def discover_tvs():
    """
    Simula el descubrimiento de TVs en la red.
    Devuelve una lista de SmartTVs.
    """
    logger.info("Recibida solicitud POST /discover")
    # Datos mockeados de TVs, similar a los que tenías en el frontend
    mock_tvs = [
        SmartTV(name='Sala de Estar TV', brand='Samsung', ip='192.168.1.10', isOnline=True, isRegistered=True),
        SmartTV(name='Dormitorio TV', brand='LG', ip='192.168.1.12', isOnline=False, isRegistered=True),
        SmartTV(name='Cocina TV', brand='Sony', ip='192.168.1.15', isOnline=True, isRegistered=False),
    ]
    return mock_tvs

@router.post("/register")
async def register_tv(tv: SmartTV):
    """
    Simula el registro de una TV.
    """
    logger.info(f"Recibida solicitud POST /register para TV: {tv.name} ({tv.ip})")
    # Aquí podrías guardar la TV en una base de datos o lista de TVs registradas
    # Por ahora, solo devolvemos un mensaje de éxito
    return {"status": "success", "message": f"TV {tv.name} registrada correctamente.", "tv": tv}

@router.post("/ping")
async def ping_tv(request: Dict[str, str]):
    """
    Simula el ping a una TV.
    """
    tv_ip = request.get("ip")
    logger.info(f"Recibida solicitud POST /ping para IP: {tv_ip}")
    # Aquí podrías intentar hacer un ping real a la IP
    # Por ahora, simulamos una respuesta
    is_online = True # Simular que siempre está online para la prueba
    return {"success": is_online, "message": f"Ping a {tv_ip}: {'exitoso' if is_online else 'fallido'}"}
...
@router.delete("/unregister/{tv_ip}")
async def unregister_tv(tv_ip: str):
    """
    Simula la desvinculación/borrado de una TV registrada.
    """
    logger.info(f"Recibida solicitud DELETE /unregister para TV con IP: {tv_ip}")
    # Aquí iría la lógica para eliminar la TV de tu base de datos o lista de TVs registradas
    # Por ahora, solo devolvemos un mensaje de éxito simulado
    return {"status": "success", "message": f"TV con IP {tv_ip} desregistrada correctamente."}

# Endpoint específico para YouTube (tu caso de uso original)
@router.post("/cast-youtube")
